  ...
  <div class="process-page">
    <nav class="navbar">
      ...
      <div class="nav-center">
        <div class="step-badge">{{ $t('processPage.stepBadge') }}</div>
        <div class="step-name">{{ $t('processPage.stepNameGraphBuild') }}</div>
      </div>
    ...
    <div class="main-content">
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('processPage.liveKnowledgeGraph') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ $t('processPage.statNodes', { count: graphData.node_count || graphData.nodes?.length || 0 }) }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ $t('processPage.statEdges', { count: graphData.edge_count || graphData.edges?.length || 0 }) }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="$t('processPage.refreshGraphTitle')">
                  <span class="icon-refresh" :class="{ 'spinning': graphLoading }">↻</span>
                </button>
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? $t('processPage.fullscreenExit') : $t('processPage.fullscreenEnter')">
        ...
        <div class="graph-container" ref="graphContainer">
          <div v-if="graphData" class="graph-view">
            ...
            <div v-if="currentPhase === 1" class="graph-building-hint">
              ...
              {{ $t('processPage.realtimeUpdating') }}
            </div>
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('processPage.graphDataLoading') }}</p>
          </div>
          
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            <div class="waiting-icon">
              <svg viewBox="0 0 100 100" class="network-icon">
                <circle cx="50" cy="20" r="8" fill="none" stroke="#64748b" stroke-width="1.5"/>
                <circle cx="20" cy="60" r="8" fill="none" stroke="#64748b" stroke-width="1.5"/>
                <circle cx="80" cy="60" r="8" fill="none" stroke="#64748b" stroke-width="1.5"/>
                <circle cx="50" cy="80" r="8" fill="none" stroke="#64748b" stroke-width="1.5"/>
                <line x1="50" y1="28" x2="25" y2="54" stroke="#64748b" stroke-width="1"/>
                <line x1="50" y1="28" x2="75" y2="54" stroke="#64748b" stroke-width="1"/>
                <line x1="28" y1="60" x2="72" y2="60" stroke="#64748b" stroke-width="1" stroke-dasharray="4"/>
                <line x1="50" y1="72" x2="26" y2="66" stroke="#64748b" stroke-width="1"/>
                <line x1="50" y1="72" x2="74" y2="66" stroke="#64748b" stroke-width="1"/>
              </svg>
            </div>
            <p class="waiting-text">{{ $t('processPage.waitingOntology') }}</p>
            <p class="waiting-hint">{{ $t('processPage.waitingOntologyHint') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('processPage.graphBuilding') }}</p>
            <p class="waiting-hint">{{ $t('processPage.graphBuildingHint') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">{{ $t('processPage.buildFlow') }}</span>
        </div>
        ...
        <div class="process-content">
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('processPage.phaseOntologyTitle') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('processPage.apiDescLabel') }}</div>
                <div class="detail-content">
                  {{ $t('processPage.phaseOntologyDesc') }}
                </div>
              </div>
              
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">{{ $t('processPage.progressLabel') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('processPage.generatedEntityTypes', { count: projectData.ontology.entity_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('processPage.generatedRelationTypes', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    {{ $t('processPage.moreRelations', { count: projectData.ontology.relation_types.length - 5 }) }}
                  </div>
                </div>
              </div>
              
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('processPage.waitingOntologyShort') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('processPage.phaseGraphTitle') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('processPage.apiDescLabel') }}</div>
                <div class="detail-content">
                  {{ $t('processPage.phaseGraphDesc') }}
                </div>
              </div>
              
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ $t('processPage.waitingOntologyDone') }}</div>
              </div>
              
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">{{ $t('processPage.buildProgressLabel') }}</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">{{ $t('processPage.buildResultsLabel') }}</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">{{ $t('processPage.resultEntityNodes') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('processPage.resultRelationEdges') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">{{ $t('processPage.resultEntityTypes') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('processPage.phaseCompleteTitle') }}</div>
                <div class="phase-api">{{ $t('processPage.phaseCompleteApi') }}</div>
              </div>
          ...
          <div class="next-step-section" v-if="currentPhase >= 2">
            <button class="next-step-btn" @click="goToNextStep" :disabled="currentPhase < 2 || creatingSimulation">
              <span v-if="!creatingSimulation">{{ $t('processPage.enterEnvSetup') }}</span>
              <span v-else>{{ $t('processPage.enterEnvSetupLoading') }}</span>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">{{ $t('processPage.projectInfo') }}</span>
          </div>
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">{{ $t('processPage.projectName') }}</span>
              <span class="item-value">{{ projectData.name }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('processPage.projectId') }}</span>
              <span class="item-value code">{{ projectData.project_id }}</span>
            </div>
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">{{ $t('processPage.graphId') }}</span>
              <span class="item-value code">{{ projectData.graph_id }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('processPage.simulationRequirement') }}</span>
...
const statusText = computed(() => {
  if (error.value) return t('processPage.statusBuildFailed')
  if (currentPhase.value >= 2) return t('processPage.statusBuildComplete')
  if (currentPhase.value === 1) return t('processPage.statusGraphBuilding')
  if (currentPhase.value === 0) return t('processPage.statusOntologyGenerating')
  return t('processPage.statusInitializing')
})
...
const goToNextStep = async () => {
  if (currentPhase.value < 2 || creatingSimulation.value) return
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('processPage.phaseDone')
  if (currentPhase.value === phase) {
    if (phase === 1 && buildProgress.value) {
      return `${buildProgress.value.progress}%`
    }
    return t('processPage.phaseActive')
  }
  return t('processPage.phasePending')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = t('processPage.errNoPendingFiles')
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: t('processPage.msgUploadingAnalyzing') }
    ...
    if (response.success) {
      ...
      error.value = response.error || t('processPage.errOntologyFailed')
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = t('processPage.errInitFailed', { error: err.message || t('common.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('processPage.errLoadProject')
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('processPage.errLoadProjectDetail', { error: err.message || t('common.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || t('processPage.errProcessFailed')
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: t('processPage.msgStartingGraphBuild')
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = t('processPage.msgGraphTaskStarted')
      ...
      error.value = response.error || t('processPage.errStartGraph')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('processPage.errStartGraphDetail', { error: err.message || t('common.unknownError') })
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || t('processPage.msgProcessing')
      }
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: t('processPage.msgBuildCompleteLoading')
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('processPage.errGraphBuildFailed', { error: task.error || t('common.unknownError') })
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('processPage.graphEmptyWait'))
    return
  }
  ...
  const nodes = nodesData.map(n => ({
    id: n.uuid,
    name: n.name || t('processPage.unnamed'),
  ...
  const edges = edgesData
    ...
    .map(e => ({
      ...
      rawData: {
        ...e,
        source_name: nodeMap[e.source_node_uuid]?.name || t('processPage.unknown'),
        target_name: nodeMap[e.target_node_uuid]?.name || t('processPage.unknown')
      }
    }))
